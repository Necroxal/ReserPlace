...
const userLogin = async (req, res, next) => {
    try {
        ...
        console.log("users.js - userLogin - data", data);
        const user = data[0]; // Sequelize returns the info as arrays.
        console.log("users.js - userLogin - user = data[0]", user)
...
const updateUserInfo = async(req, res, next) => {
    try {
        ...
        const data = getUserByEmail(req.body.email);   // Get the user to update.

        updateUser(data[0], newData); // Update the user's info.
